...
\end{equation}
which is used here as a representative value selected for this aerospace vibration test case. The use of base-excitation vibration tests is consistent with aerospace payload verification practice~\cite{nasa_std_7002b}, and the selected amplitude is small enough to keep the structural response in the linear regime assumed by the modal-superposition harmonic solver~\cite{inman_engineering_vibration}. The complete definition of the base acceleration and the Fixed Support in ANSYS Mechanical is shown in \Cref{fig:base_acceleration}.
...
\end{equation}
applied as \emph{constant modal damping} (a single ratio common to all modes)~\cite{inman_engineering_vibration}. The three resulting datasets are compared in \Cref{sec:damping}.
